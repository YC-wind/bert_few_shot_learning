...
config = {
    ...
    "out": "./fsl_match_cos/",  # 保存模型路径
    "out_1": "./fsl_match_cos/"  # 保存模型路径
}
...
def create_model(bert_config, is_training, input_ids_support,
    ...
    print("use cos_distance...")
    # [B , total_Q,  N * K]
    cos_distance = get_cos_distance_batch(query, support)
    cos_distance = tf.reshape(cos_distance, [-1, total_q, config["N"], config["K"]])
    # [B , total_Q,  N]
    cos_distance = tf.reduce_max(cos_distance, -1, name="cos_distance")
    print("cos_distance:{}".format(cos_distance))
    # [B * total_Q,  N]
    logits = tf.reshape(cos_distance, [-1, config["N"]])
    # [B * total_Q,  N]
    pred = tf.cast(tf.greater_equal(logits, tf.constant(0.5, dtype=tf.float32)), tf.int32, name="predictions")
    print("predictions:{}".format(pred))
    # 这里使用均方误差作为损失(感觉也不妥，其他概率就要最小化到0)
